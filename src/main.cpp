...
std::vector<uint8_t> GetPieceTable(Storage &s, const std::vector<uint8_t> &wds) {
  ...
  s.read_stream(((get16(wds, 10) & 0x0200) == 0x0200) ? "1Table" : "0Table", ts);
...
int main(int argc, char *argv[]) {
  ...
  if (s.read_stream("WordDocument", str)) {
    std::vector<uint8_t> pt = GetPieceTable(s, str); //Piece Table
    WriteFromPieceTable(tmp_file, pt, str);
  } else if (s.read_stream("PowerPoint Document", str)) {
    WriteStream(tmp_file, str);
  } else if (s.read_stream("Workbook", str)) {
    WriteStream(tmp_file, str);
  }
  fclose(tmp_file);
